<template>
  <section class="container mx-auto py-16">
    <h2 class="text-5xl font-bold text-center mb-8 text-[#011751]">
      Heat resistant coating And weather resistant paints
    </h2>
    <div
      class="grid grid-cols-1 lg:grid-cols-2 gap-8 text-center lg:max-w-4xl xl:max-w-5xl 2xl:max-w-7xl sm:max-w-2xl mx-auto mt-24"
    >
    <div>
    <img src="~/assets/images/heat.png" class="p-2 sm:w-[200rem]" />
    <p class="text-start pt-8 text-[#011751] font-serif text-xl lg:block hidden">
          We use only the most advanced materials from Zahabiya Chemical
          Industries and application techniques, ensuring that every surface we
          treat is reinforced to withstand the harshest conditions, guaranteeing
          the longevity of your building
        </p>
    </div>
      <div class="p-4">
        <h3 class="font-bold text-start text-3xl mb-2 text-[#33628C] uppercase">
          Heat resistant
        </h3>
        <p class="text-start font-semibold text-xl font-serif pt-8">
          where high temperatures and intense sun exposure are common,
          heatresistant coatings are crucial for enhancing building performance.
          These coatings reflect solar heat, reducing indoor temperatures and
          improving comfort while lowering air conditioning costs. They also
          minimize the expansion and contraction of materials caused by extreme
          heat, preventing structural damage and prolonging the lifespan of
          buildings
        </p>
        <p class="text-start pt-8 text-[#011751] font-serif text-xl">
          By enhancing thermal insulation, heat-resistant coatings lower energy
          consumption and utility bills, promoting sustainability in
          construction. Additionally, these coatings are designed to withstand
          harsh weather conditions, reducing maintenance needs and extending the
          life of roofs and surfaces. Available in various colors and finishes,
          heat-resistant coatings also enhance the visual appeal of properties
          while providing
        </p>
        <p class="text-start pt-8 text-[#011751] font-serif text-xl lg:hidden">
          We use only the most advanced materials from Zahabiya Chemical
          Industries and application techniques, ensuring that every surface we
          treat is reinforced to withstand the harshest conditions, guaranteeing
          the longevity of your building
        </p>
      </div>
    </div>
  </section>
</template>